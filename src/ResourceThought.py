...
class ResourceThought (TextThought.TextThought):
    def __init__(self, coords, pango_context, thought_number, save, undo, loading, background_color, foreground_color):
        super(ResourceThought, self).__init__(coords, pango_context, thought_number,
                                              save, undo, loading, background_color, foreground_color, "res_thought")
        ...
        self.dialog = self.glade.get_object('ResourceChooserDialog')
    ...
    def process_uri_dialog(self, initial=True):
        ...
        if res == Gtk.ResponseType.OK:
            ...
            self.uri = self.glade.get_object('urlEntry').get_text()
            if initial:
    ...
    def process_button_down(self, event, mode, transformed):
        ...
        if event.type == Gdk.ModifierType.BUTTON_PRESS and not self.editing:
            self.emit("select_thought", event.state & modifiers)
        # Gdk._2BUTTON_PRESS:
        if event.button == 1 and mode == BaseThought.MODE_TEXT and event.type == Gdk.ModifierType.BUTTON2_MASK:
    ...
    def load(self, node):
        ...
        self.glade.get_object('urlEntry').set_text(self.uri)

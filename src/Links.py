...
class Link (GObject.GObject):
    ...
    def __init__(self, save, parent=None, child=None, start_coords=None, end_coords=None, strength=2):
        ...
        self.selected = False
        self.color = utils.gtk_to_cairo_color(Gdk.Color.parse("black")[1])
    ...
    def draw(self, context):
        ...
        if utils.use_bezier_curves:
            ...
            context.curve_to(
                x2, self.start[1], x3, self.end[1], self.end[0], self.end[1])
        else:
    ...
    def export(self, context, move_x, move_y):
        ...
        if utils.use_bezier_curves:
            ...
            x2 = self.start[0] + dx / 2.0
            x3 = self.end[0] - dx / 2.0
            context.curve_to(x2+move_x, self.start[1]+move_y, x3+move_x,
                             self.end[1]+move_y, self.end[0]+move_x, self.end[1]+move_y)
        else:
    ...
    def load(self, node):
        ...
        try:
            ...
            self.color = (float(colors[0].strip('(,)')), float(
                colors[1].strip('(,)')), float(colors[2].strip('(,)')))
        except:
            pass

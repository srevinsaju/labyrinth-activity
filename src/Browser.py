...
class Browser (Gtk.Window):
    ...
    def __init__(self, start_hidden, tray_icon):
        ...
        self.glade.add_from_fi(utils.get_data_file_name('labyrinth.glade'))
        ...
        self.glade.get_object('import1').connect(
            'activate', self.import_clicked)
        ...
        self.glade.get_object('showhelp').connect(
            'activate', self.show_help_clicked)

        list(map(lambda x: x.set_sensitive(False), self.view_dependants))
        ...
        if os.name != 'nt':
            ...
            self.config_client.add_dir(
                "/apps/labyrinth", gconf.CLIENT_PRELOAD_NONE)
        ...
        if os.name != 'nt':
            sort_order = self.config_client.get_int(
                '/apps/labyrinth/map_sort_order')
            column_id = self.config_client.get_int(
                '/apps/labyrinth/map_sort_order_column')
        ...
        if tray_icon:
            self.main_window.connect(
                'delete_event', self.toggle_main_window, None)
            traymenu = Gtk.Menu()
    ...
    def cursor_change_cb(self, treeview):
        ...
        list(map(lambda x: x.set_sensitive(sensitive), self.view_dependants))
    ...
    def about_clicked(self, arg):
        ...
        if os.name != 'nt':
            ...
            about_dialog.set_logo(
                GdkPixbuf.Pixbuf.new_from_file("images\\labyrinth-24.png"))
    ...
    def delete_clicked(self, button):
        ...
        if map.window:
            error_message = _(
                "The map cannot be deleted right now. Is it open?")
    ...
    def sort_column_changed_cb(self, data):
        ...
        if os.name != 'nt':
            self.config_client.set_int(
                '/apps/labyrinth/map_sort_order', sort_order)
            self.config_client.set_int(
                '/apps/labyrinth/map_sort_order_column', column_id)

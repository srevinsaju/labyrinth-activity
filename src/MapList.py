...
class MapList(object):
    ...
    class MapCore(object):
        ...
        def __getattr__(self, key):
            ...
            if key in dict:
                ...
                raise ValueError(
                    "Class MapCore does not have an attribute named %s" % key)
    ...
    def __init__(self):
        raise Exception(
            "This class is a singleton full of classmethods, dont instantiate it.")
    ...
    def new_from_file(cls, filename):
        ...
        map.modtime = datetime.datetime.fromtimestamp(
            os.stat(filename)[8]).strftime("%x %X")
        cls.tree_view_model.append(
            [map.index, map.title, map.modtime, map.filename, False])

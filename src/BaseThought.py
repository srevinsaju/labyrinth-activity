...
class BaseThought (GObject.GObject):
    ...
    def __init__(self, save, elem_type, undo, background_color, foreground_color):
        ...
        self.extended_buffer = TextBufferMarkup.ExtendedBuffer(
            self.undo, extended_elem, save)
...
# FIXME
CURSOR[RESIZE_LEFT] = Gdk.CursorType.LEFT_SIDE
CURSOR[RESIZE_RIGHT] = Gdk.CursorType.RIGHT_SIDE
CURSOR[RESIZE_TOP] = Gdk.CursorType.TOP_SIDE
CURSOR[RESIZE_BOTTOM] = Gdk.CursorType.BOTTOM_SIDE
CURSOR[RESIZE_LEFT | RESIZE_TOP] = Gdk.CursorType.TOP_LEFT_CORNER
CURSOR[RESIZE_LEFT | RESIZE_BOTTOM] = Gdk.CursorType.BOTTOM_LEFT_CORNER
CURSOR[RESIZE_RIGHT | RESIZE_TOP] = Gdk.CursorType.TOP_RIGHT_CORNER
CURSOR[RESIZE_RIGHT | RESIZE_BOTTOM] = Gdk.CursorType.BOTTOM_RIGHT_CORNER
...
class ResizableThought (BaseThought):
    ...
    def __init__(self, coords, save, elem_type, undo, background_color, foreground_color):
        super(ResizableThought, self).__init__(
            save, elem_type, undo, background_color, foreground_color)
    ...
    def handle_motion(self, event, coords):
        ...
        if self.ul[0] > self.lr[0]:
            ...
            self.resizing = (~self.resizing & 0x3) | (
                self.resizing & (0x3 << 2))
        ...
        if self.ul[1] > self.lr[1]:
            ...
            self.resizing = (~self.resizing & (0x3 << 2)
                             ) | (self.resizing & 0x3)
    ...
    def undo_resize(self, action, mode):
        ...
        self.pic = self.orig_pic.scale_simple(
            int(self.width), int(self.height), GdkPixbuf.InterpType.HYPER)
        self.recalc_edges()
        self.emit("update_links")
        self.emit("update_view")
        self.undo.unblock()

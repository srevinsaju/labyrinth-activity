...
class DrawingThought(ResizableThought):
    ...
    def __init__(self, coords, pango_context, thought_number, save, undo, loading, background_color, foreground_color):
        global ndraw
        super(DrawingThought, self).__init__(coords, save,
                                             "drawing_thought", undo, background_color, foreground_color)
        ndraw += 1
    ...
    def handle_motion(self, event, coords):
        ...
        if self.drawing == 1:
            ...
            if len(self.points) == 0 or self.points[-1].style == STYLE_END:
                p = self.DrawingPoint(
                    coords, STYLE_BEGIN, self.foreground_color)
            else:
            ...
            for x in self.points:
                ...
                if dist < 16:
                    ...
                    if len(loc) != 0:
                        ...
                        y1 = x.y - out.y
                        d_rsqr = x1**2 + y1 ** 2
                        d = ((out.x-coords[0])*(x.y-coords[1]) -
                             (x.x-coords[0])*(out.y-coords[1]))
                        det = (d_rsqr*16) - d**2
                        if det > 0:
                            xt = -99999
                            yt = -99999
                            ...
                            x1_inside = (xt > x.x and xt < out.x) or (
                                xt > out.x and xt < x.x)
                            x2_inside = (xalt > x.x and xalt < out.x) or (
                                xalt > out.x and xalt < x.x)
                            y1_inside = (yt > x.y and yt < out.y) or (
                                yt > out.y and yt < x.y)
                            ...
                            if (x1_inside and x2_inside and y1_inside and y2_inside):
                                if abs(xalt - x.x) < abs(xt - x.x):
                                    handle.append(
                                        (None, out, x, ins_point, xt, xalt, yt, yalt))
                                else:
                                    handle.append(
                                        (None, out, x, ins_point, xalt, xt, yalt, yt))
    ...
    def move_content_by(self, x, y):
        list(map(lambda p: p.move_by(x, y), self.points))
    ...
    def update_save(self):
        ...
        self.element.setAttribute(
            "foreground-color", self.foreground_color.to_string())

...
from src.BaseThought import BaseThought
from src import utils
from src import UndoManager
from src import ResourceThought
from src import DrawingThought
from src import ImageThought
from src import LabelThought
from src import TextThought
...
class MMapArea (Gtk.DrawingArea):
    ...
    def __init__(self, undo):
        ...
        self.save = impl.createDocument(
            "http://www.donscorgie.blueyonder.co.uk/labns", "MMap", None)
        ...
        utils.default_colors["text"] = utils.gtk_to_cairo_color(c.get_color(
            Gtk.StateFlags.NORMAL))  # utils.gtk_to_cairo_color(style.text[gtk.STATE_NORMAL])
        utils.default_colors["base"] = utils.gtk_to_cairo_color(c.get_background_color(
            Gtk.StateFlags.NORMAL))  # utils.gtk_to_cairo_color(style.base[gtk.STATE_NORMAL])
        ...
        utils.default_colors["bg"] = utils.gtk_to_cairo_color(c.get_background_color(
            Gtk.StateFlags.NORMAL))  # utils.gtk_to_cairo_color(style.bg[gtk.STATE_NORMAL])
        utils.default_colors["fg"] = utils.gtk_to_cairo_color(c.get_color(
            Gtk.StateFlags.NORMAL))  # utils.gtk_to_cairo_color(style.fg[gtk.STATE_NORMAL])

        utils.selected_colors["text"] = utils.gtk_to_cairo_color(c.get_color(
            Gtk.StateFlags.SELECTED))  # utils.gtk_to_cairo_color(style.text[gtk.STATE_SELECTED])
        utils.selected_colors["bg"] = utils.gtk_to_cairo_color(c.get_background_color(
            Gtk.StateFlags.SELECTED))  # utils.gtk_to_cairo_color(style.bg[gtk.STATE_SELECTED])
        utils.selected_colors["fg"] = utils.gtk_to_cairo_color(c.get_color(
            Gtk.StateFlags.SELECTED))  # utils.gtk_to_cairo_color(style.fg[gtk.STATE_SELECTED])
        utils.selected_colors["fill"] = utils.gtk_to_cairo_color(c.get_background_color(
            Gtk.StateFlags.SELECTED))  # utils.gtk_to_cairo_color(style.base[gtk.STATE_SELECTED])
    ...
    def button_down(self, widget, event):
        ...
        coords = self.transform_coords(
            event.get_coords()[0], event.get_coords()[1])
    ...
    def motion(self, widget, event):
        ...
        if event.state & Gdk.ModifierType.BUTTON1_MASK and self.is_bbox_selecting:
            ...
            for t in self.selected:
                t.move_by(coords[0] - self.move_origin_new[0],
                          coords[1] - self.move_origin_new[1])
    ...
    def make_primary(self, thought):
        ...
        self.title_change_handler = thought.connect(
            "title_changed", self.title_changed_cb)
        ...
        if thought:
            try:
                self.commit_handler = self.im_context.connect(
                    "commit", thought.commit_text, self.mode, None, None)
    ...
    def select_link(self, link, modifiers):
        ...
        if modifiers and (modifiers & Gdk.ModifierType.SHIFT_MASK or modifiers == -1):
            ...
            list(map(lambda t: t.unselect(), self.selected))
            self.selected = [link]
        link.select()
    ...
    def select_thought(self, thought, modifiers):
        ...
        if modifiers and (modifiers & Gdk.ModifierType.SHIFT_MASK or modifiers == -1):
            ...
            list(map(lambda x: x.unselect(), self.selected))
        ...
        thought.select()
        if len(self.selected) == 1:
            self.emit("thought_selection_changed",
                      thought.background_color, thought.foreground_color)
    ...
    def update_all_links(self):
        list(map(lambda l: l.find_ends(), self.links))
    ...
    def invalidate(self, transformed_area=None):
        ...
        if not transformed_area:
            ...
            ul = self.untransform_coords(
                transformed_area[0], transformed_area[1])
            lr = self.untransform_coords(
                transformed_area[2], transformed_area[3])
            rect = Gdk.Rectangle(int(ul[0]), int(
                ul[1]), int(lr[0]-ul[0]), int(lr[1]-ul[1]))
        if self.window:
    ...
    def create_new_thought(self, coords, type=None, loading=False):
        ...
        if type == MODE_TEXT:
            ...
            thought = TextThought.TextThought(coords, self.pango_context, self.nthoughts, self.save, self.undo, loading, self.background_color,
                                              self.foreground_color, fixed=self.get_parent().get_parent().get_parent().get_parent(), parent=self)
        elif type == MODE_LABEL:
            thought = LabelThought.LabelThought(coords, self.pango_context, self.nthoughts,
                                                self.save, self.undo, loading, self.background_color, self.foreground_color)
        elif type == MODE_IMAGE:
            thought = ImageThought.ImageThought(coords, self.pango_context, self.nthoughts,
                                                self.save, self.undo, loading, self.background_color, self.foreground_color)
        elif type == MODE_DRAW:
            ...
            thought = ResourceThought.ResourceThought(
                coords, self.pango_context, self.nthoughts, self.save, self.undo, loading, self.background_color, self.foreground_color)
        if not thought.okay():
            return None
    ...
    def delete_thought(self, thought, undo=True):
        if undo:
            action = UndoManager.UndoAction(
                self, UNDO_DELETE_SINGLE, self.undo_deletion, [thought])
        else:
            action = None
            ...
            for t in action.args[0]:
                ...
                if t.am_primary and not self.primary:
                    self.emit("change_buffer",
                              action.args[0][0].extended_buffer)
                    self.make_primary(t)
        else:
            for t in action.args[0]:
    ...
    def delete_selected_elements(self):
        if len(self.selected) == 0:
            return
        action = UndoManager.UndoAction(
            self, UNDO_DELETE, self.undo_deletion, copy.copy(self.selected))
...
# print " , "+str(math.pi / 3.0)+" , "+str(currentangle)+"\n: "+str(relangle)
    ...
    def align_top_left(self, vertical=True):
        ...
        self.undo.add_undo(UndoManager.UndoAction(
            self, UNDO_ALIGN, self.undo_align, dic))
    ...
    def align_centered(self, vertical=True):
        ...
        if len(self.selected) != 0:
            x = self.selected[0].ul[0] + \
                (self.selected[0].lr[0] - self.selected[0].ul[0]) / 2.0
            y = self.selected[0].ul[1] + \
                (self.selected[0].lr[1] - self.selected[0].ul[1]) / 2.0
            for t in self.selected:
                if vertical:
    ...
    def link_menu_cb(self):
        ...
        if lnk:
            self.undo.add_undo(UndoManager.UndoAction(
                self, UNDO_DELETE_LINK, self.undo_link_action, lnk))
            self.delete_link(lnk)
        else:
            lnk = self.create_link(self.selected[0], None, self.selected[1])
            self.undo.add_undo(UndoManager.UndoAction(
                self, UNDO_CREATE_LINK, self.undo_link_action, lnk))
        self.invalidate()

...
class LabyrinthWindow (GObject.GObject):
    ...
    def __init__(self, filename, imported=False):
        ...
        glade = Gtk.Builder()
        builder.add_from_file(utils.get_data_file_name('labyrinth.glade'))
        ...
        self.cut = self.ui.get_object('/MenuBar/EditMenu/Cut')
        self.copy = self.ui.get_object('/MenuBar/EditMenu/Copy')
        self.paste = self.ui.get_object('/MenuBar/EditMenu/Paste')
        self.link = self.ui.get_object('/MenuBar/EditMenu/LinkThoughts')
        self.delete = self.ui.get_object('/MenuBar/EditMenu/DeleteNodes')

        self.ui.get_object(
            '/MenuBar/EditMenu').connect('activate', self.edit_activated_cb)
        self.cut.set_sensitive(False)
        self.copy.set_sensitive(False)

        # get toolbars and activate corresponding menu entries
        self.main_toolbar = self.ui.get_object('/ToolBar')
        self.format_toolbar = glade.get_object('format_toolbar')
        self.ui.get_object(
            '/MenuBar/ViewMenu/ShowToolbars/ShowMainToolbar').set_active(True)
        self.ui.get_object(
            '/MenuBar/ViewMenu/ShowToolbars/ShowFormatToolbar').set_active(True)
        self.ui.get_object(
            '/MenuBar/ViewMenu/UseBezier').set_active(utils.use_bezier_curves)
        ...
        self.align_button = glade.get_object('tool_align')
        self.align_handler_id = self.align_button.connect(
            'clicked', self.align_cb, "vl")
        glade.get_object('align_vertical_left').connect(
            'activate', self.align_cb, "vl")
        glade.get_object('align_vertical_right').connect(
            'activate', self.align_cb, "vr")
        glade.get_object('align_vertical_center').connect(
            'activate', self.align_cb, "vc")
        glade.get_object('align_horizontal_top').connect(
            'activate', self.align_cb, "ht")
        glade.get_object('align_horizontal_bottom').connect(
            'activate', self.align_cb, "hb")
        glade.get_object('align_horizontal_center').connect(
            'activate', self.align_cb, "hc")
    ...
    def show(self):
        ...
        if os.name != 'nt':
            if not self.config_client.get_bool('/apps/labyrinth/show_tree_view'):
                ...
                self.ui.get_object(
                    '/MenuBar/ViewMenu/ViewOutline').set_active(True)
    ...
    def create_tree_view(self, glade):
        ...
        self.tree_view = glade.get_object('outline_treeview')
    ...
    def align_cb(self, widget, direction):
        ...
        if widget != self.align_button:
            ...
            self.align_handler_id = self.align_button.connect(
                'clicked', self.align_cb, direction)
            self.align_button.set_icon_name(
                widget.get_image().get_icon_name()[0])
    ...
    def view_extend_cb(self, arg):
        self.undo.add_undo(UndoManager.UndoAction(
            self, UNDO_SHOW_EXTENDED, self.undo_show_extended))
    ...
    def view_outline_cb(self, arg):
        if arg.get_active():
            ...
            self.tree_view.hide()
        if os.name != 'nt':
            self.config_client.set_bool(
                '/apps/labyrinth/show_tree_view', arg.get_active())
    ...
    def view_change_cb(self, base, activated):
        ...
        if os.name != 'nt':
            self.config_client.set_bool(
                '/apps/labyrinth/curves', utils.use_bezier_curves)
    ...
    def translate(self, box, arg1, direction):
        self.orig_translate = [
            self.MainArea.translation[0], self.MainArea.translation[1]]
        if direction == "Up":
            translation_x = 0
            translation_y = 5
        elif direction == "Down":
            ...
            print("Error")
            return
        GObject.timeout_add(20, self.translate_timeout,
    ...
    def doc_save_cb(self, widget, doc, top_element):
        ...
        if not self.save_file:
            sham = hashlib.sha256(save_string).hexdigest()
            ...
            while os.path.exists(self.save_file):
                ...
                self.save_file = save_loc + "Dup" + \
                    str(counter)+sham.hexdigest()+".map"
                counter += 1
    ...
    def export_map_cb(self, event):
        chooser = Gtk.FileChooserDialog(title=_("Save File As"), action=Gtk.FILE_CHOOSER_ACTION_SAVE,
                                        buttons=(Gtk.STOCK_CANCEL, Gtk.ResponseType.CANCEL, Gtk.STOCK_OPEN, Gtk.ResponeType.OK))
        chooser.set_current_name("%s.mapz" % self.main_window.title)
        response = chooser.run()
        if response == Gtk.ResponeType.OK:
    ...
    def export_cb(self, event):
        ...
        if mime in ['png', 'jpg']:
            self.save_as_pixmap(filename, mime, true_width,
                                true_height, bitdepth, native)
        else:
            surface = None
            if mime == 'svg':
